...
const ExperienceItem = ({ link, name, desc, image, startDate, endDate }) => (
  <li className="dark:text-zinc-350 my-5 flex items-center gap-4 text-zinc-500">
    <a
      ...
      target="_blank"
    >
      <Image alt={name} src={image} className="h-14 w-24 rounded-md" />
    </a>
    <div className="flex min-w-0 flex-col justify-center">
      <p className="mb-1 flex items-center">
        <span className="truncate font-semibold text-zinc-700 dark:text-zinc-100">{name}</span>
      ...
      <p className="flex items-center truncate">
        <span className="flex-1 truncate text-zinc-500 dark:text-zinc-400">{desc}</span>
      </p>
    </div>
  </li>
)
